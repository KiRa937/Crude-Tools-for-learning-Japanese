...
-- -- This version has 2 more features
-- -- 1) Script adds tag with the video file name
-- -- 2) Script can replace old Audio and image
-- -- To replace the Audio first add tag "ac-rep" to the note in question
-- -- and make sure to either deselect it in Browse window or close the window completely
-- -- !!!ONLY ONE NOTE MUST HAVE THIS TAG!!!
-- -- Select all the subtitle lines you wish to add to the card like in the Instructions above.
-- -- Ctrl + c
-- -- Tab back to MPV and Ctrl + r
-- -- Done. The lines, their respective Audio and the current paused image are replaced
-- -- and the tag is deleted unless the note was selected in Browse window at the time
...
local prefix = utils.join_path(os.getenv('APPDATA'), [[Anki2\K\collection.media]])
...
local function add_to_last_added(ifield, afield, tfield, ep, mode)
  ...
  local added_notes = anki_connect('findNotes', {query=qR})["result"]
...
local function get_extract(mode)
  ...
  if e ~= 0 then
	...
	local ep = mp.get_property("filename"):gsub('%W','')
	
    local forvo_path = add_to_last_added(ifield, afield, tfield, ep, mode)
...
mp.add_key_binding("ctrl+r", "replace-anki-card", re)
